...
interface MemorySegment {
  ...
  sentiment?: number; // -1 to 1 scale for sentiment analysis
}
...
export class EnhancedMemoryManager {
  ...
  public async getEnhancedContext(
    ...
    if (userProfile) {
      ...
      if (userProfile.emotionalPatterns) {
        userPreferencesContext += '- Emotional patterns:\n';
        userPreferencesContext += `  - Dominant emotions: ${[...userProfile.emotionalPatterns.dominantEmotions.entries()].map(([emotion, count]) => `${emotion} (${count})`)} \n`;
        userPreferencesContext += `  - Average intensity: ${userProfile.emotionalPatterns.averageIntensity.toFixed(2)}\n`;
      }
  ...
  private async updateUserProfile(
    ...
    await this.ensureUserProfile(sessionId);
    ...
    if (emotionalContext) {
      ...
      emotionalContext.emotions.forEach(emotion => {
        const count = profile.emotionalPatterns.dominantEmotions.get(emotion) || 0;
        profile.emotionalPatterns.dominantEmotions.set(emotion, count + 1);
      });

      // Update intensity tracking
      const oldTotal = profile.emotionalPatterns.averageIntensity * profile.emotionalPatterns.interactionCount;
      profile.emotionalPatterns.interactionCount++;
      profile.emotionalPatterns.averageIntensity = (oldTotal + emotionalContext.intensity) / profile.emotionalPatterns.interactionCount;
    }
  ...
  /**
   * Extract emotional context from a message (e.g., using sentiment analysis)
   * @param message The message to analyze
   * @returns Emotional context or undefined if not available
   */
  private async extractEmotionalContext(message: string): Promise<{ emotions: string[]; intensity: number } | undefined> {
    try {
      // Placeholder for a sentiment analysis call
      // Replace with actual sentiment analysis API call or logic
      // This example just simulates some emotional context
      const simulatedEmotions = ['happy', 'excited'];
      const simulatedIntensity = 0.7;

      return { emotions: simulatedEmotions, intensity: simulatedIntensity };
    } catch (error) {
