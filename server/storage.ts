...
/**
 * Interface for canvas suggestions generated by the AI research agent
 */
...
export interface IStorage {
  ...
  // Canvas suggestions methods (for AI research agent)
  getCanvasSuggestions(canvasId: string): Promise<CanvasSuggestion[]>;
  saveCanvasSuggestions(canvasId: string, suggestions: CanvasSuggestion[]): Promise<boolean>;
...
export class MemStorage implements IStorage {
  ...
  // Canvas suggestions methods (for AI research agent)
  async getCanvasSuggestions(canvasId: string): Promise<CanvasSuggestion[]> {
    return this.canvasSuggestions.get(canvasId) || [];
  }
  
  async saveCanvasSuggestions(canvasId: string, suggestions: CanvasSuggestion[]): Promise<boolean> {
    try {
      // Ensure each suggestion has a timestamp if not already present
      const timestampedSuggestions = suggestions.map(suggestion => ({
        ...suggestion,
        timestamp: suggestion.timestamp || Date.now()
      }));
      
      this.canvasSuggestions.set(canvasId, timestampedSuggestions);
      return true;
    } catch (error) {
      console.error("Error saving canvas suggestions:", error);
      return false;
    }
  }
